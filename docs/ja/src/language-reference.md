...
ポリシーで許可されたコマンドと pure function call は `async` で並行実行できます。
式の型は `Future[T]` です。`T` は包んだ呼び出しの戻り値型です。`await` は future
を待機して捕捉した値を返します。コマンドが失敗した場合は、その終了ステータスで
伝播します。
...
raw shell のバックグラウンド実行は引き続き無効です。`async` が受け付けるのは、
安全な approved command call と pure function call だけです。
...
- 生の Bash ブロック
- シェルのパイプラインとリダイレクト
- safe `async` approved-command/function call 以外のバックグラウンド、非同期、spawn
  形式のシェルコマンド
